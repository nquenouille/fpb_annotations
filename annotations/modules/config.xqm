...
declare variable $config:webcomponents-cdn := "/apps/annotations/resources/scripts/";
...
(:
 ...
 :)
declare variable $config:default-template :="annotate.html";
...
declare variable $config:context-path :=
   if (not(empty(request:get-header("X-Forwarded-Host")))) then 
        ("") 
    else 
        (request:get-context-path() || substring-after($config:app-root, "/db"))
;
...
(:~
 ...
 :)
declare variable $config:default-odd :="annotations.odd";
...
(:~
 ...
 :)
declare variable $config:odd-available :=("annotations.odd", "FPB.odd");
