                ...
                return
                    if (xmldb:collection-available($collectionPath)) then
                        if (ends-with($path, ".docx")) then
                            ...
                            (: Setting permissions - The code is a slightly modified version of the one at https://github.com/daliboris/theatrum-neolatinum-register-data/blob/main/modules/upload.xqm#L244-L257, kindly provided by Boris Lehečka :)
                            for $document in collection($collectionPath)
                                let $file-permission := "rw-rw-r--"
                                let $file-group := "tei"
                                let $filename := util:document-name($document)
                                return (sm:chmod(base-uri($document), $file-permission), sm:chgrp(base-uri($document), $file-group)))
                    else
