    ...
    return
        typeswitch($node)
            ...
            case element(tei:note) return
                if ($node[@type='presentation'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='received'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='possessor'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='address'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='numbering'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='postalNote'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                else if ($node[@type='processed'] is $target) then
                    element exist:delete {
                        $node/@*,
                        anno:delete($node/node(), $target)
                    }
                ...
                else if ($node[@type='note'] is $target) then
                    ()
                else if ($node is $target) then
                    element exist:delete {
                        $node/@*,
        ...
        typeswitch($node)
            ...
            case element(tei:signed) return
                if ($node is $target) then
                    element { node-name($node) } {
                        map:for-each($annotation?properties, function($key, $value) {
                            if ($value != '' and $key='handSigned') then
                                attribute {'hand'} {$value}
                            else if ($value != '' and $key='ref') then
                                attribute {'hand'} {$value}
                            else
...
declare %private function anno:apply($node as node(), $startOffset as xs:int, $endOffset as xs:int, $annotation as map(*)) {
    ...
    let $endAdjusted :=
        if ($end?2 = string-length($end?1) and not($start?1 is $end?1)) then
            ...
            let $offset := if ($outer/following-sibling::node() and not($outer/following-sibling::tei:lb)) then $end?2 else if($outer/following-sibling::node()) then 1 else $end?2
            return
        ...
        return
            typeswitch($node)
                ...
                case element(tei:note) return
                    ...
                    let $found := anno:find-offset($primary, $offset+1, $pos, ()) (: added + anno:string-length($node) for better offset :)
                    return
                        if (exists($found)) then
                            $found
                        else
                    ...
                    else anno:find-offset(tail($nodes), $offset - anno:string-length($node), $pos, ())
